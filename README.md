...
| `https://llm-agent.mwm.local:21443/toollab` | Tool Lab — 도구 등록/편집 (Phase 07, `TOOLLAB_ALLOWED_GROUPS` 게이트) |
| `https://llm-agent.mwm.local:21443/toollab/run` | Tool Run — 도구 picker + 자연어 실행 + multi-turn trace |
| `https://grafana.mwm.local:21443/` | Grafana (OIDC SSO, 대시보드 `LLM Admin` / `LLM User` / Tool Lab) |
...
| `LLM Admin` | LLM 사용자 / 트레이스 드릴다운 | Top 20 사용자 + user_id 셀 클릭 → 같은 dashboard 자동 필터 (data link). `toollab_run_request` 도 포함 |
| `LLM Admin` | Tool Lab | run 수·tool call 수·도구별 호출 빈도·평균 iterations·truncated 비율·`model_id × tool_call_parser` 비교·도구 에러 분포·`toollab_tool_parser_leak` 알람 |
...
- [`docs/배포가이드.md`](docs/배포가이드.md) — 단일 호스트 운영 배포 체크리스트
- [`docs/운영이관_가이드.md`](docs/운영이관_가이드.md) — dev 단일 compose → prod 분리(중앙 Loki/Grafana, 외부 vLLM/PG/Qdrant, secret 관리) 이관 절차
